...
// 나중에도 고쳐서 사용하기 위해 필요 없는 부분은 지우지 않고 주석처리 했습니다.
...
public:
    ...
    GLfloat piraNormals[48] = {  // 72 elements
        ...
        0, 1, 0,   0, 1, 0,   0, 1, 0,              // v0,v5,v6,v1 (top)
        0, 1, 0,   0, 1, 0,   0, 1, 0,              // v0,v5,v6,v1 (top)
    ...
    */
    ...
    void initBuffers() {
        ...
        glBufferData(GL_ARRAY_BUFFER, vSize+cSize, 0, GL_STATIC_DRAW); // reserve space                         +nSize+tSize
        ...
        glBufferSubData(GL_ARRAY_BUFFER, vSize, cSize, piraColors);          // copy cols after norms           +nSize
        ...
        glVertexAttribPointer(2, 4, GL_FLOAT, GL_FALSE, 4 * sizeof(float), (void *)(vSize)); //color attrib     +nSize
        //glVertexAttribPointer(3, 2, GL_FLOAT, GL_FALSE, 2 * sizeof(float), (void *)(vSize+cSize)); // tex coord +nSize
